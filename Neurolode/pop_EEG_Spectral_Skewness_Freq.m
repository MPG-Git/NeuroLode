...
% pop_EEG_Spectral_Skewness_Freq
% Time-resolved spectral skewness using octave-band power (frequency domain).
%
% Usage:
%   >> [EEG, com] = pop_EEG_Spectral_Skewness_Freq(EEG)   % GUI
%   >> [EEG, com] = pop_EEG_Spectral_Skewness_Freq(EEG,'1:5 8',0,1,200,20,1)
%
% Inputs:
...
% - For each frame: compute octave-band power with POCTAVE, then spectral
%   skewness across octave centers → one scalar per frame.
...
if useGUI
    ...
    uilist = { ...
        {'style','text','string','Spectral Skewness — Frequency Domain','fontweight','bold'} ...
    ...
    res = inputgui(geom, uilist, 'title','Spectral Skewness — Frequency Domain');
...
if size(X,1) == 1
    ...
    for f = 1:nFrames
        ...
        c(f) = spectralSkewness(p(:).', cf(:).');% skewness across octave centers
    end
    C = c;
...
if any(isnan(C),'all')
    error('Neurolode Error: invalid skewness values (check window/overlap).');
end

% ------------- plot (GUI only) -------------
if useGUI
    figure('Name','Spectral Skewness (Frequency Domain)');
    plot(T, C.'); grid on;
    xlabel('Time (s)'); ylabel('Skewness');
    legend(labels, 'Location','northwest');
    title(sprintf('Freq-domain skewness (Window %d ms, Overlap %d ms)', ...
...
if ExportData
    ...
    fname = sprintf('%s_SpectralSkewness_Freq_%s.xlsx', base, tag);
    try
        writecell(sheet, fname);
    catch
        try
            fname = sprintf('%s_SpectralSkewness_Freq_%s.csv', base, tag);
            writecell(sheet, fname);
        catch
            fname = sprintf('%s_SpectralSkewness_Freq_%s.txt', base, tag);
...
if useGUI
    ...
    com = sprintf('EEG = pop_EEG_Spectral_Skewness_Freq(EEG,%s,%d,%d,%g,%g,1);', ...
