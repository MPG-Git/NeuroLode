...
% pop_EEG_Spectral_Spread_Freq
% Frequency-domain spectral spread over time using melSpectrogram.
%
% Usage:
%   >> [EEG, com] = pop_EEG_Spectral_Spread_Freq(EEG)  % GUI
%   >> [EEG, com] = pop_EEG_Spectral_Spread_Freq(EEG,'1:5 Cz',1,1,200,20,1)
%
% Inputs:
...
%   Frames2buffer        window length in ms for melSpectrogram (e.g., 200)
...
if useGUI
    ...
    uilist = { ...
        {'style','text','string','Spectral Spread — Frequency Domain (melSpectrogram)','fontweight','bold'} ...
    ...
    res = inputgui(geom, uilist, 'title','Spectral Spread — Frequency Domain');
...
mustPos    = @(x,n) assert(isnumeric(x)&&isscalar(x)&&isfinite(x)&&x>0, '%s must be positive.', n);
...
% -------- Compute frequency-domain spread over time --------
% Using melSpectrogram for time-frequency representation; then spectralSpread(s, cf).
if size(X,1) == 1
    [S, cf, T] = melSpectrogram(double(X(:)), fs, ...
        'WindowLength', winSamp, 'OverlapLength', ovlSamp);
    SS = spectralSpread(S, cf);            % 1 x nFrames
    C  = SS(:).';                          % rows=channels (1), cols=time
    ...
    T = [];
    for k = 1:nC
        [Sk, cf, Tk] = melSpectrogram(double(X(k,:)).', fs, ...
            'WindowLength', winSamp, 'OverlapLength', ovlSamp);
        SSk = spectralSpread(Sk, cf);
        ssk = SSk(:).';
        if isempty(C)
            C = ssk;
            T = Tk(:).';
        else
            L = min(size(C,2), numel(ssk));   % align lengths across channels
            C = [C(:,1:L); ssk(1:L)];
            T = T(1:L);
        end
    end
...
if any(isnan(C),'all')
    error('Neurolode Error: invalid spread values. Check window/overlap.');
end

% -------- Plot (GUI only) --------
if useGUI
    figure('Name','Spectral Spread (Frequency Domain)');
    plot(T, C.'); grid on;
    xlabel('Time (s)'); ylabel('Spread (Hz)');
    legend(labels, 'Location','northwest');
    title(sprintf('Spectral Spread (Window %d ms, Overlap %d ms)', round(Frames2buffer), round(OverlapFrames)));
end
...
if ExportData
    ...
    fname = sprintf('%s_SpectralSpread_Freq_%s.xlsx', base, tag);
    try
        writecell(sheet, fname);
    catch
        try
            fname = sprintf('%s_SpectralSpread_Freq_%s.csv', base, tag);
            writecell(sheet, fname);
        catch
            fname = sprintf('%s_SpectralSpread_Freq_%s.txt', base, tag);
...
if useGUI
    ...
    com = sprintf('EEG = pop_EEG_Spectral_Spread_Freq(EEG,%s,%d,%d,%g,%g,1);', ...
