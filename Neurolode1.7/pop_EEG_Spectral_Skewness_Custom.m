...
% pop_EEG_Spectral_Skewness_Custom
% Time-resolved spectral skewness with custom window/overlap/frequency range.
%
% Usage:
%   >> [EEG, com] = pop_EEG_Spectral_Skewness_Custom(EEG)  % GUI
%   >> [EEG, com] = pop_EEG_Spectral_Skewness_Custom(EEG,'1:5',1,1,200,20,[1 40],1)
%
% Inputs:
...
if useGUI
    ...
    uilist = { ...
        {'style','text','string','Spectral Skewness — Custom','fontweight','bold'} ...
    ...
    res = inputgui(geom, uilist, 'title','Spectral Skewness — Custom');
...
if size(X,1) == 1
    ...
    T = [];
    for k = 1:nC
        [SS, tSec] = spectralSkewness(double(X(k,:)).', fs, ...
            'Window', win, 'OverlapLength', ovlSamp, 'Range', [fmin fmax]);
        ssk = SS(:).';
        if isempty(C)
            C = ssk;
...
if any(isnan(C),'all')
    error('Neurolode Error: invalid skewness values. Check HzRange/window/overlap.');
end

% ---------------- plot (GUI only) ----------------
if useGUI
    figure('Name','Spectral Skewness (Custom)');
    plot(T, C.'); grid on;
    xlabel('Time (s)'); ylabel('Skewness');
    legend(labels, 'Location','northwest');
    title(sprintf('Spectral Skewness (Window %d ms, Overlap %d ms, Range [%g %g] Hz)', ...
...
if ExportData
    ...
    fname = sprintf('%s_SpectralSkewness_Custom_%s.xlsx', base, tag);

    try
        writecell(sheet, fname);
    catch
        try
            fname = sprintf('%s_SpectralSkewness_Custom_%s.csv', base, tag);
            writecell(sheet, fname);
        catch
            fname = sprintf('%s_SpectralSkewness_Custom_%s.txt', base, tag);
...
if useGUI
    ...
    com = sprintf(['EEG = pop_EEG_Spectral_Skewness_Custom(EEG,%s,%d,%d,%g,%g,[%g %g],1);'], ...
