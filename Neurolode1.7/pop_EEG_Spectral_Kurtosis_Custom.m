...
% pop_EEG_Spectral_Kurtosis_Custom
% Time-resolved spectral kurtosis with custom window/overlap/frequency range.
%
% Usage:
%   >> [EEG, com] = pop_EEG_Spectral_Kurtosis_Custom(EEG)  % GUI
%   >> [EEG, com] = pop_EEG_Spectral_Kurtosis_Custom(EEG,'1:5',1,1,200,20,[1 40],1)
%
% Inputs:
...
if useGUI
    ...
    uilist = { ...
        {'style','text','string','Spectral Kurtosis — Custom','fontweight','bold'} ...
        ...
        {'style','text','string','Frequency range [min max] Hz'} ...
        {'style','edit','string','[1 40]'} ...
        {'style','text','string','e.g., [1 40]'} ...
    };
    res = inputgui(geom, uilist, 'title','Spectral Kurtosis — Custom');
    ...
    HzRange = str2num(res{6}); %#ok<ST2NM>
else
    COIraw = COI;
...
assert(isnumeric(HzRange)&&numel(HzRange)==2&&all(isfinite(HzRange))&&HzRange(1)<HzRange(2), ...
    'HzRange must be [fmin fmax] with fmin < fmax.');
...
% helper to build time vector from number of frames (center times)
mkT = @(nFrames) ((0:nFrames-1)*hop + winSamp/2)/fs;

if size(X,1) == 1
    % one trace
    sk = spectralKurtosis(double(X(:)), fs, ...
        'Window', win, 'OverlapLength', ovlSamp, 'Range', [fmin fmax]);
    C = sk(:).';                       % 1 x F
    T = mkT(numel(C));                 % 1 x F (s)
    ...
    T = [];
    for k = 1:nC
        sk = spectralKurtosis(double(X(k,:)).', fs, ...
            'Window', win, 'OverlapLength', ovlSamp, 'Range', [fmin fmax]);
        sk = sk(:).';
        if isempty(C)
            C = sk;
...
if any(isnan(C),'all')
    error('Neurolode Error: invalid kurtosis values. Check HzRange/window/overlap.');
end

% ---------------- plot (GUI only) ----------------
if useGUI
    figure('Name','Spectral Kurtosis (Custom)');
    plot(T, C.'); grid on;
    xlabel('Time (s)'); ylabel('Kurtosis');
    legend(labels, 'Location','northwest');
    title(sprintf('Spectral Kurtosis (Window %d ms, Overlap %d ms, Range [%g %g] Hz)', ...
        round(Frames2buffer), round(OverlapFrames), fmin, fmax));
end
...
if ExportData
    ...
    fname = sprintf('%s_SpectralKurtosis_Custom_%s.xlsx', base, tag);

    try
        writecell(sheet, fname);
    catch
        try
            fname = sprintf('%s_SpectralKurtosis_Custom_%s.csv', base, tag);
            writecell(sheet, fname);
        catch
            fname = sprintf('%s_SpectralKurtosis_Custom_%s.txt', base, tag);
...
if useGUI
    ...
    com = sprintf(['EEG = pop_EEG_Spectral_Kurtosis_Custom(EEG,%s,%d,%d,%g,%g,[%g %g],1);'], ...
