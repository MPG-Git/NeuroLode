...
% pop_EEG_Spectral_Spread_Custom
% Time-resolved spectral spread with custom window/overlap/frequency range.
%
% Usage:
%   >> [EEG, com] = pop_EEG_Spectral_Spread_Custom(EEG)  % GUI
%   >> [EEG, com] = pop_EEG_Spectral_Spread_Custom(EEG,'1:5',1,1,200,20,[1 40],1)
%
% Inputs:
...
if useGUI
    ...
    uilist = { ...
        {'style','text','string','Spectral Spread — Custom','fontweight','bold'} ...
    ...
    res = inputgui(geom, uilist, 'title','Spectral Spread — Custom');
...
% ---------------- compute spread ----------------
% spectralSpread returns per-frame spread and time vector when given a vector.
if size(X,1) == 1
    [SS, T] = spectralSpread(double(X(:)), fs, ...
    ...
    T = [];
    for k = 1:nC
        [SSk, tSec] = spectralSpread(double(X(k,:)).', fs, ...
            'Window', win, 'OverlapLength', ovlSamp, 'Range', [fmin fmax]);
        ssk = SSk(:).';
        if isempty(C)
            C = ssk;
            ...
            L = min(size(C,2), numel(ssk));   % align lengths
            C = [C(:,1:L); ssk(1:L)];
            T = T(1:L);
        end
    end
...
if any(isnan(C),'all')
    error('Neurolode Error: invalid spread values. Check HzRange/window/overlap.');
end

% ---------------- plot (GUI only) ----------------
if useGUI
    figure('Name','Spectral Spread (Custom)');
    plot(T, C.'); grid on;
    xlabel('Time (s)'); ylabel('Spread (Hz)');
    legend(labels, 'Location','northwest');
    title(sprintf('Spectral Spread (Window %d ms, Overlap %d ms, Range [%g %g] Hz)', ...
...
if ExportData
    ...
    fname = sprintf('%s_SpectralSpread_Custom_%s.xlsx', base, tag);
    try
        writecell(sheet, fname);
    catch
        try
            fname = sprintf('%s_SpectralSpread_Custom_%s.csv', base, tag);
            writecell(sheet, fname);
        catch
            fname = sprintf('%s_SpectralSpread_Custom_%s.txt', base, tag);
...
if useGUI
    ...
    com = sprintf('EEG = pop_EEG_Spectral_Spread_Custom(EEG,%s,%d,%d,%g,%g,[%g %g],1);', ...
