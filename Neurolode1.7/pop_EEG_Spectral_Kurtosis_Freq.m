...
% pop_EEG_Spectral_Kurtosis_Freq
% Time-resolved spectral kurtosis using octave-band power (frequency domain).
%
% Usage:
%   >> [EEG, com] = pop_EEG_Spectral_Kurtosis_Freq(EEG)   % GUI
%   >> [EEG, com] = pop_EEG_Spectral_Kurtosis_Freq(EEG,'1:5 8',0,1,200,20,1)
%
% Inputs:
...
%
% Notes:
% - Epochs (3rd dim) are averaged first to get one continuous vector per channel.
% - For each frame, computes octave-band power via POCTAVE, then spectral
%   kurtosis across octave centers → one scalar per frame.
...
if useGUI
    ...
    uilist = { ...
        {'style','text','string','Spectral Kurtosis — Frequency Domain','fontweight','bold'} ...
    ...
    res = inputgui(geom, uilist, 'title','Spectral Kurtosis — Frequency Domain');
...
if size(X,1) == 1
    ...
    for f = 1:nFrames
        ...
        [p, cf] = poctave(seg, fs);              % power per octave band & center freqs
        c(f) = spectralKurtosis(p(:).', cf(:).');% kurtosis across octave centers
    end
    C = c;
...
if any(isnan(C),'all')
    error('Neurolode Error: invalid kurtosis values (check window/overlap).');
end

% ------------- plot (GUI only) -------------
if useGUI
    figure('Name','Spectral Kurtosis (Frequency Domain)');
    plot(T, C.'); grid on;
    xlabel('Time (s)'); ylabel('Kurtosis');
    legend(labels, 'Location','northwest');
    title(sprintf('Freq-domain kurtosis (Window %d ms, Overlap %d ms)', ...
...
if ExportData
    ...
    fname = sprintf('%s_SpectralKurtosis_Freq_%s.xlsx', base, tag);
    try
        writecell(sheet, fname);
    catch
        try
            fname = sprintf('%s_SpectralKurtosis_Freq_%s.csv', base, tag);
            writecell(sheet, fname);
        catch
            fname = sprintf('%s_SpectralKurtosis_Freq_%s.txt', base, tag);
...
if useGUI
    ...
    com = sprintf('EEG = pop_EEG_Spectral_Kurtosis_Freq(EEG,%s,%d,%d,%g,%g,1);', ...
